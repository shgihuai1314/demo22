...
class owtModel extends model
{
    ...
    public function getConfiguration($type = '')
    {
        ...
        $owt = new stdClass();
        $owt->enabled    = $this->setting->getItem("owner=system&module=owt&section=common&key=enabled");
        $owt->serviceId  = $this->setting->getItem("owner=system&module=owt&section=common&key=serviceid");
        $owt->serviceKey = $this->setting->getItem("owner=system&module=owt&section=common&key=servicekey");
        $owt->serverAddr = $this->setting->getItem("owner=system&module=owt&section=common&key=serveraddr");
        $owt->apiPort    = $this->setting->getItem("owner=system&module=owt&section=common&key=apiport");
        $owt->mgmtPort   = $this->setting->getItem("owner=system&module=owt&section=common&key=mgmtport");

        if($type == 'client')
        {
            if($owt->enabled == 'false' || empty($owt->serverAddr) || empty($owt->serviceId) || empty($owt->serviceKey)) return false;
    ...
    public function setConfiguration()
    {
        ...
        $errors = array();
        foreach(array('serviceId', 'serviceKey', 'serverAddr', 'apiPort', 'serviceKey') as $item)
        {
